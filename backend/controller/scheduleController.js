...
const addSchedules = async (req, res) => {
  const { selectedDate, schedulesData } = req.body; // Get the selected date and schedules data from the request body
  ...
  try {
    ...
    weekEnd.setDate(weekEnd.getDate() + 7); // End of the week (Saturday)

    // Calculate the start and end of the month
    const monthStart = new Date(year, month, 1); // Start of the month
    const monthEnd = new Date(year, month + 1, 0); // End of the month

    // Prepare the schedules to be added
    const schedules = schedulesData.map((schedule) => ({
      name: schedule.name,
      shift_schedule: schedule.shift_schedule
        .filter((shift) => {
        ...
        .map((shift) => ({
          date: shift.date, // Keep the date as a string
          title: shift.title,
          public: shift.public,
          shiftContainer: shift.shiftContainer,
          shiftStart: shift.shiftStart,
          shiftStop: shift.shiftStop,
          Vacation: shift.Vacation,
        })),
    }));

    // Filter out any schedules that have no shifts to add
    const validSchedules = schedules.filter(
      (schedule) => schedule.shift_schedule.length > 0
    );

    // Insert the valid schedules into the database
    await Schedule.insertMany(validSchedules);
